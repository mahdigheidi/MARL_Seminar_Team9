...
Previously, we have seen environments and agents in which we have used independent learning algorithms such as IDQN. Problem? The huge number of parameters and the slow training process.
...
\begin{block}{}
    When having strongly homogeneous agents, we know that all agents use identical policies. So do we need to keep different parameters for each?
\end{block}
\vspace{1em}
    \centering
\begin{adjustbox}{max width=\textwidth,max height=0.6\textheight}
\begin{tikzpicture}[
    >=Latex,
...
\begin{tikzpicture}[
    card/.style={
        ...
        fill=cardfill,
        minimum width=4cm, minimum height=0.5cm,
    ...
    caption/.style={align=center, font=\large, text width=5.5cm}
]
...
\node[card] (b1t) at (-4.5,2.42) {$(s_1,a_1,r_1,s_2')$};
\node[dots] (b1d) at (-4.5,1.9) {$\vdots$};
\node[card] (b1b) at (-4.5,1.38) {$(s_{k},a_{k},r_{k},s_{k+1}')$};
...
\node[card] (b2t) at (-4.5,0.02) {$(s_1,a_1,r_1,s_2')$};
\node[dots] (b2d) at (-4.5,-0.5) {$\vdots$};
\node[card] (b2b) at (-4.5,-1.02) {$(s_{k},a_{k},r_{k},s_{k+1}')$};
\begin{pgfonlayer}{background}
...
\node[card] (b3t) at (-4.5,-2.78) {$(s_1,a_1,r_1,s_2')$};
\node[dots] (b3d) at (-4.5,-3.3) {$\vdots$};
\node[card] (b3b) at (-4.5,-3.82) {$(s_{k},a_{k},r_{k},s_{k+1}')$};
\begin{pgfonlayer}{background}
\node[container, fit=(b3t)(b3b), inner sep=6pt] (buf3) {};
\end{pgfonlayer}
\node[font=\small] at (buf3.west) [anchor=east, xshift=-4pt] {Agent $K$};

\node[caption] at (-4.5,-5.5) {Each agent stores transitions in its own replay buffer};
...
\node[card] (s1) at (4.3,1.62) {$(s_1,a_1,r_1,s_2')$};
\node[card] (s2) at (4.3,1.10) {$(s_2,a_2,r_2,s_3')$};
\node[dots]      at (4.3,0.60) {$\vdots$};
\node[card] (s3) at (4.3,0.10) {$(s_{k-1},a_{k-1},r_{k-1},s_k')$};
\node[card] (s4) at (4.3,-0.42) {$(s_k,a_k,r_k,s_{k+1}')$};
\begin{pgfonlayer}{background}
...
\[
\mathbb{E}_{a \sim \pi}\!\left[ f(a) \right]
=
\mathbb{E}_{a \sim \pi_\beta}\!\left[
...
\begin{tikzpicture}[>=Latex]
    % gradient bar from own-color (0) to shared-color (1)
    \shade[left color=seacown, right color=seacshared]
        (0,0) rectangle (7,0.35);
    \draw[thick] (0,0) rectangle (7,0.35);

    % end ticks and numeric labels
    \draw[thick] (0,0) -- (0,-0.12);
    \draw[thick] (7,0) -- (7,-0.12);
    \node[below, font=\small] at (0,-0.12) {$\lambda = 0$};
    \node[below, font=\small] at (7,-0.12) {$\lambda = 1$};

    % descriptive captions under each end
    \node[below, font=\scriptsize, text=seacown,   align=center] at (0,-0.55)
        {no sharing};
    \node[below, font=\scriptsize, text=seacshared, align=center] at (7,-0.55)
        {others' experience\\treated as own};

    % lambda label above the bar
    \node[above, font=\small] at (3.5,0.35) {$\lambda$};
\end{tikzpicture}
\end{center}
\end{frame}